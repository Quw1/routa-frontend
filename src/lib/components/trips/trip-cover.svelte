<script lang="ts">
	...
	const dateLocal = new Date(trip.start_date);
</script>
	...
	<a href="trips/{trip.id}">
		<ContextMenu.Root>
			...
			<ContextMenu.Content class="w-40">
				<ContextMenu.Item><a href="trips/{trip.id}">Edit locations</a></ContextMenu.Item>
				<ContextMenu.Separator />
				<ContextMenu.Item
					on:click={() => {
						dialogOpen = true;
					}}>Delete trip</ContextMenu.Item
				>
			</ContextMenu.Content>
		</ContextMenu.Root>
		<div class="mt-3 space-y-1">
			...
			<p class="text-md text-muted-foreground">
				{dateLocal.toLocaleDateString('en-US', dateOptions)}
			</p>
		</div>
	</a>
</div>
